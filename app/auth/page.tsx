			...
			<Form onSubmit={onSubmit} ref={formRef} noValidate>
				...
				<FormContent>
					<FormLabel>
						{t("Email")}
						{(() => {
							const buttonText = countdown > 0 ? `00:${countdown.toString().padStart(2, "0")}` : t("Send");
							...
							return (
								<InputWithButtonWrapper $padRight={paddingRight}>
									<input
										...
										required
									/>
									<ActionInsideButton type="button" onClick={handleSendCode} disabled={countdown > 0 || !email}>
										{buttonText}
									</ActionInsideButton>
								</InputWithButtonWrapper>
							);
