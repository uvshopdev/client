...
export default function Home() {
	...
	const productSections = [t("New arrivals"), t("Monthly discounts"), t("Top rated products"), t("Best sellers")];
		...
		<S.Main>
			...
			<S.HeroSection>
				...
				<S.Controls>
					...
					<S.Dots>
						{slides.map((_, i) => (
							<S.Dot key={crypto.randomUUID()} $active={i === current} />
						))}
					</S.Dots>
			...
			<S.Section>
				...
				<S.CategoriesGrid>
					{Array.from({ length: 6 }).map(() => (
						<Link key={crypto.randomUUID()} href="/catalog">
							<S.CategoryCard>
								<S.CategoryCircle />
			...
			<S.SectionTight>
				...
				<S.CountryBanner src="/images/countryMonth.png" alt={t("Countries of the month")} />
			</S.SectionTight>
			...
			{/* PRODUCT SECTIONS */}
			{productSections.map((title) => (
				<S.Section key={crypto.randomUUID()}>
					<S.Header>
