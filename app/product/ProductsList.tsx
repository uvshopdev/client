...
const ProductsList = ({ currentProductId }: ProductsListProps) => {
    ...
    useEffect(() => {
        if (categories.length > 0 && randomProductCategories.length === 0) {
            const categoriesWithProducts = categories.filter((c) => c.category_id !== null);
            const source = categoriesWithProducts.length > 0 ? categoriesWithProducts : categories;
